...
# Add arguments for specifying TCP algorithm, flow length, and delay
parser.add_argument('--tcp', type=str, default="cubic", help="Specify the TCP algorithm to be used (default: cubic)")
parser.add_argument('--length', type=int, default=200, help="Duration of the second flow in seconds (default: 200)")
parser.add_argument('--delay', type=int, default=50, help="Delay after which the second flow starts in seconds (default: 50)")

# Parse the argument
args = parser.parse_args()
total_length = args.length + args.delay
...
# This program runs for 250 seconds (default) and creates a new directory called
# `TCP 2 up Delay(date-timestamp)_dump`. It contains a `README` that
...
# Refer https://github.com/tohojo/flent/blob/master/flent/tests/tcp_2up_delay.conf
...
flow1 = Flow(h1, h3, eth3.get_address(), 0, total_length, 1)
flow2 = Flow(h1, h3, eth4.get_address(), args.delay, total_length, 1)
flow3 = Flow(h2, h4, eth3.get_address(), 0, total_length, 1)
flow4 = Flow(h2, h4, eth4.get_address(), args.delay, total_length, 1)
