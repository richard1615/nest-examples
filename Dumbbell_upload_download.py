...
parser.add_argument('--tcp1', type=str, default="cubic")

parser.add_argument('--tcp2', type=str, default="bbr")



parser.add_argument('--num_upload_flows', type=int, default=2)

parser.add_argument('--num_download_flows', type=int, default=2)
...
num_upload_flows = args.num_upload_flows

num_download_flows = args.num_download_flows
...
for i in range(1, num_upload_flows + num_download_flows + 1):

    if i <= num_upload_flows:

        src_host = h1 if i % 2 == 1 else h2

        dest_host = h3 if i <= num_upload_flows // 2 else h4

        src_eth = eth3 if i % 2 == 1 else eth4

    else:

        src_host = h3 if i <= num_upload_flows + num_download_flows // 2 else h4

        dest_host = h1 if i % 2 == 1 else h2

        src_eth = eth3 if i <= num_upload_flows + num_download_flows // 2 else eth4



    flow = Flow(src_host, dest_host, src_eth.get_address(), 0, 200, 1)

    flows.append(flow)



    # Configure the upload and download flows

    exp.add_tcp_flow(flow, args.tcp1 if i % 2 == 1 else args.tcp2)
