...
parser.add_argument('--num_uploadstream', type=int, default=1)
# Parse the argument
...
exp = Experiment("tcp upload")

num_upload_stream = args.num_uploadstream


# Configure upload from `h1` to `h3` and from `h2` to `h4`.
flow1 = Flow(h1, h3, eth3.get_address(), 0, 200, num_upload_stream)
flow2 = Flow(h1, h3, eth3.get_address(), 0, 200, num_upload_stream)
flow3 = Flow(h2, h4, eth4.get_address(), 0, 200, num_upload_stream)
flow4 = Flow(h2, h4, eth4.get_address(), 0, 200, num_upload_stream)
...
exp.add_tcp_flow(flow4, args.tcp2)
