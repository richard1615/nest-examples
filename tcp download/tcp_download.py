...
parser.add_argument('--num_downloadstream', type=int, default=1)
...
exp = Experiment("tcp download")


num_d = args.num_downloadstream

#Configure download from `h3` to `h1` and from `h4` to `h2`.
flow1 = Flow(h3, h1, eth1.get_address(), 0, 200, num_d)
flow2 = Flow(h3, h1, eth1.get_address(), 0, 200, num_d)
flow3 = Flow(h4, h2, eth2.get_address(), 0, 200, num_d)
flow4 = Flow(h4, h2, eth2.get_address(), 0, 200, num_d)
