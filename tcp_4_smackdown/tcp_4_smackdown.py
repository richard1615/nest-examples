...
# Add an argument
parser.add_argument('--tcp1', type=str, default="reno")
parser.add_argument('--tcp2', type=str, default="cubic")
parser.add_argument('--tcp3', type=str, default="westwood")
parser.add_argument('--tcp4', type=str, default="cdg")
...
# - `h4` via two routers `r1` and `r2`. Four TCP flows are configured from `h1` to
# `h3` and `h2` to `h4`.  The links `h1` <--> `r1`,
...
# h1 --------------------|                          |-------------------- h3 #
#  tcp1,tcp2,tcp3 & tcp4 |                          | tcp1,tcp2,tcp3 & tcp4  #
#                        |    <- 10mbit, 10ms ->    |                        #
#                         r1 -------------------- r2                         #
#                        |                          |                        #
#  tcp1,tcp2,tcp3 & tcp4 |                          | tcp1,tcp2,tcp3 & tcp4  #
...
# `tcp_4_smackdown(date-timestamp)_dump`. It contains a `README` that
...
exp = Experiment("tcp_4_smackdown")
...
flow3 = Flow(h1, h3, eth3.get_address(), 0, 200, 1)
flow4 = Flow(h1, h3, eth3.get_address(), 0, 200, 1)

flow5 = Flow(h2, h4, eth4.get_address(), 0, 200, 1)
flow6 = Flow(h2, h4, eth4.get_address(), 0, 200, 1)
flow7 = Flow(h2, h4, eth4.get_address(), 0, 200, 1)
flow8 = Flow(h2, h4, eth4.get_address(), 0, 200, 1)

# Configure download from `h3` to `h1` and from `h4` to `h2`.
flow9 = Flow(h3, h1, eth1.get_address(), 0, 200, 1)
flow10 = Flow(h3, h1, eth1.get_address(), 0, 200, 1)
flow11 = Flow(h3, h1, eth1.get_address(), 0, 200, 1)
flow12 = Flow(h3, h1, eth1.get_address(), 0, 200, 1)

flow13 = Flow(h4, h2, eth2.get_address(), 0, 200, 1)
flow14 = Flow(h4, h2, eth2.get_address(), 0, 200, 1)
flow15 = Flow(h4, h2, eth2.get_address(), 0, 200, 1)
flow16 = Flow(h4, h2, eth2.get_address(), 0, 200, 1)

# Use `flow1` as a tcp1 flow,'flow2' as tcp2 flow,'flow3' as tcp3 flow and 'flow4' as tcp4 flow
# Use `flow5` as a tcp1 flow,'flow6' as tcp2 flow,'flow7' as tcp3 flow and 'flow8' as tcp4 flow
# Use `flow9` as a tcp1 flow,'flow10' as tcp2 flow,'flow11' as tcp3 flow and 'flow12' as tcp4 flow
# Use `flow13` as a tcp1 flow,'flow14' as tcp2 flow,'flow15' as tcp3 flow and 'flow16' as tcp4 flow

exp.add_tcp_flow(flow1, args.tcp1)
exp.add_tcp_flow(flow2, args.tcp2)
exp.add_tcp_flow(flow3, args.tcp3)
exp.add_tcp_flow(flow4, args.tcp4)

exp.add_tcp_flow(flow5, args.tcp1)
exp.add_tcp_flow(flow6, args.tcp2)
exp.add_tcp_flow(flow7, args.tcp3)
exp.add_tcp_flow(flow8, args.tcp4)

exp.add_tcp_flow(flow9, args.tcp1)
exp.add_tcp_flow(flow10, args.tcp2)
exp.add_tcp_flow(flow11, args.tcp3)
exp.add_tcp_flow(flow12, args.tcp4)

exp.add_tcp_flow(flow13, args.tcp1)
exp.add_tcp_flow(flow14, args.tcp2)
exp.add_tcp_flow(flow15, args.tcp3)
exp.add_tcp_flow(flow16, args.tcp4)
